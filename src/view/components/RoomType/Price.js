...
import { currentReservationAtom, displayAtom, reservationListAtom, roomTypeListAtom } from '../../../data/state'
...
export default function Price({ price, targetIndex, reservation }) {
  ...
  const [{ isOver, canDrop }, drop] = useDrop(
    () => ({
      ...
      canDrop: (item, monitor) => {
        ...
        //canDrop은 true false만 있을뿐이다
        //리스트에서 인덱스로 못가는곳을 구분하자
        //
        //1.예약 겹치지 않게하기
        //item이 드래그중인 source고 인덱스가 target이다
        //list에서 일단 갈 수 없는 인덱스를 뽑자 다뽑은다음에 자신만 빼면된다
  ...
  return (
    <div ref={drop}>
      <div style={{ display: 'grid', position: 'relative', width: '100%', height: '100%' }}>
        <a href='/#' className='none' style={{ display: 'block', placeSelf: 'center' }} ref={ref}>
          {`${price}만`}
        </a>

        {/* reservation */}
        {reservation?.startIndex === targetIndex && !isDragging && <ReservationOverlay data={reservation} drag={drag} />}
        {/* hover overlay */}
